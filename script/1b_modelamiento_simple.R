...
act_ppal_network_inputs <- pre_process(df, text_variable =  "act_principal",
                                       label =  codigo_final, type = "sequences",
                                       maxlen = maxlen_act_ppal)
# tareas_network_inputs <- pre_process(df, text_variable =  "tareas_proc", label =  aeciuo_2, type = "sequences",
...
#
# oficio_max_pool = oficio_model %>% layer_global_max_pooling_1d()
# oficio_ave_pool = oficio_model %>% layer_global_average_pooling_1d()
#
...
#   layer_gru(units = 150, return_sequences = TRUE)
#
# tareas_max_pool = tareas_model %>% layer_global_max_pooling_1d()
# tareas_ave_pool = tareas_model %>% layer_global_average_pooling_1d()
...
output = layer_concatenate(list(act_ppal_max_pool, act_ppal_ave_pool), trainable = F) %>%
  layer_dense(embedding_dim, activation='relu') %>%
  layer_dropout(0.5) %>%
  layer_dense(units = clases, activation = "softmax")

# output = layer_concatenate(list(oficio_max_pool, oficio_ave_pool, tareas_max_pool, tareas_ave_pool)) %>%
#   layer_dense(embedding_dim, activation='relu') %>%
#   layer_dropout(0.5) %>%
#   layer_dense(units = clases, activation = "softmax")

# model = keras_model(list(oficio_input, tareas_input), output)
...
cbs= list(callback_early_stopping(verbose = T, patience=15, monitor='val_loss'),
          callback_model_checkpoint(filepath=glue('data/output/modelo/mejor_modelo.h5'), monitor='val_loss',
                                    save_best_only = T, verbose = T))
tf$random$set_seed(104)
# history <- model %>%
#   fit(list(oficio_network_inputs$x_train %>% data.matrix, tareas_network_inputs$x_train %>% data.matrix),
#       oficio_network_inputs$y_train,
#       epochs = 100,
#       validation_data = list(list(oficio_network_inputs$x_test, tareas_network_inputs$x_test), oficio_network_inputs$y_test),
#       batch_size = 256,
...
modelo = load_model_hdf5('data/output/modelo/mejor_modelo.h5')
keys = act_ppal_network_inputs$keys

x_test = texts_to_sequences(act_ppal_tokenizer, act_ppal_network_inputs$df_test$act_principal) %>%
